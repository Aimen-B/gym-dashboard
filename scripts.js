document.addEventListener("DOMContentLoaded", function () {
  ...
  function generateLockerElement(locker) {
    ...
    lockerNumber.className =
      "absolute inset-0 flex justify-center items-center text-white text-lg";
    lockerNumber.innerText = locker.id;

    const maintenanceButton = document.createElement("button");
    maintenanceButton.className =
      "absolute top-2 right-2 p-1 text-white rounded-full fa-solid fa-triangle-exclamation fa-xl";
  ...
  function toggleMaintenance(lockerId) {
    ...
    if (locker) {
      locker.status =
        locker.status === "maintenance" ? "available" : "maintenance";
  ...
  function updateLockerStatus(lockerId, status) {
    ...
    if (lockerElement) {
      lockerElement.classList.remove(
        "bg-green-500",
        "bg-blue-700",
        "bg-red-500"
      );
